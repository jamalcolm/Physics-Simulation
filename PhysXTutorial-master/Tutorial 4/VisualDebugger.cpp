...
namespace VisualDebugger
{
	...
	void RenderScene()
	{
		...
		hud.AddLine(HELP, "Test case: " + scene->GetCurrentTestCase());
		hud.AddLine(HELP, "Box percent:" + std::to_string(scene->boxPercent * 100));
		hud.AddLine(HELP, "no. of boxes:" + std::to_string(scene->boxes.size()));
		hud.AddLine(HELP, "no. of balls:" + std::to_string(scene->noOfBalls));
		hud.AddLine(HELP, "FPS: " + std::to_string(physicsFPS.GetFPS()));
		hud.AddLine(HELP, "FPS_avg: " + std::to_string(physicsFPS.GetAvgFPS()));
	...
	void ForceInput(int key)
	{
		...
		PxRigidDynamic* selectedActor = scene->GetSelectedActor();
		switch (toupper(key))
		{
			// Force controls on the selected actor
		case 'I': //forward

			selectedActor->addForce(PxVec3(0,0,-1)*gForceStrength);
			break;
		case 'K': //backward
			selectedActor->addForce(PxVec3(0,0,1)*gForceStrength);
			break;
		case 'J': //left
			selectedActor->addForce(PxVec3(-1,0,0)*gForceStrength);
			break;
		case 'L': //right
			selectedActor->addForce(PxVec3(1,0,0)*gForceStrength);
			break;
		case 'U': //up
			selectedActor->addForce(PxVec3(0,1,0)*gForceStrength);
			break;
		case 'M': //down
			selectedActor->addForce(PxVec3(0,-1,0)*gForceStrength);
			break;
